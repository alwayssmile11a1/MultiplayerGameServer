...
int UDPSocket::Bind(const SocketAddress& inBindAddress)
{
	...
	if (error != 0)
	{
		NetworkHelper::ReportError("UDPSocket::Bind");
...
int UDPSocket::SendTo(const void* inToSend, int inLength, const SocketAddress& inToAddress)
{
	...
	if (byteSentCount <= 0)
	{
		...
		NetworkHelper::ReportError("UDPSocket::SendTo");
		return -NetworkHelper::GetLastError();
	}
	else
	{
...
int UDPSocket::ReceiveFrom(void* inToReceive, int inMaxLength, SocketAddress& outFromAddress)
{
	...
	{
		int error = NetworkHelper::GetLastError();
		...
		{
			NetworkHelper::ReportError("UDPSocket::ReceiveFrom");
			return -error;
		}
	}
}
...
int UDPSocket::SetNonBlockingMode(bool inShouldBeNonBlocking)
{
	...
	{
		NetworkHelper::ReportError("UDPSocket::SetNonBlockingMode");
		return NetworkHelper::GetLastError();
	}
	else
	{
